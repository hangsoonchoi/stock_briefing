...
**압축형으로 짧게.** 각 포지션 카드는 **최대 3줄, 200토큰 이내**:
- stock-card (data-section="position-update")
- 헤더 1줄: "📌 보유" / "✅ 익절 도달" / "🚨 손절 도달" / "⚠️ 손절 임박" / "📉 단기 하락"
- live-price-row (추천가/현재가/변동) 1줄
- 권고 1줄만:
  - 손절 → "**즉시 매도 검토**" (warning)
  - 임박 → "**매도 준비**"
  - 익절 → "**일부 매도 검토**" (discovery)
  - 보유 → "**보유 유지**"

stock-prices 영역(매수가/익절1/익절2/손절) 표시 안 해도 됨 — 이미 추적 중이니까.
stock-reason도 1줄로 압축 — "오늘 -2% 빠짐, 손절가 근접" 정도.

evaluated_positions 비어있으면 "오늘 아침 풀 분석 데이터 없음 — 새 추천부터 시작" 한 줄로 끝.

**이 섹션이 다른 섹션을 잡아먹지 않게 짧게.** 메인 섹션은 매수 검토 후보·300만원 분산이다.
