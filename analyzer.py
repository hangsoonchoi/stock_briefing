...
총 약 28,000 토큰 (한도 32K). 모든 섹션 디테일 유지:

- TL;DR: 300
- 💸 지금 팔아야 할 것: 카드당 350 × N
- 🚫 절대 사지 말 것: 카드당 350 × 3 = 1,050
- 🏆 TOP 3 우선순위: 카드당 600 × 3 = 1,800
- 📌 아침 추천 포지션: 카드당 350 × 6 = 2,100
- ⚡ 단타 후보: 카드당 350 × 3 = 1,050
- 🎯 매수 검토: 카드당 500 × 6 = 3,000
- 💰 300만원 분산: 카드당 400 × 6 = 2,400
- 🔍 발굴 후보: 카드당 200 × 8 = 1,600
- 🌍 세계+거시: 1,200
- 🚀 톱무버: 600
- 🏭 섹터: 500
- 📋 관심종목 평가: 종목당 150 × 7 = 1,050
- 📰 뉴스: 600
- ⚠️ 조심+면책: 400
- 📚 용어 풀이: 600

**16개 섹션 모두 디테일하게 출력. 잘리지 않게 구성.**
...
<h2>💸 지금 당장 팔아야 할 것 (SELL — 손실 막기)</h2>

evaluated_positions 중 **손절 도달 / 손절 임박 / 큰 하락 / 악재 발생** 종목만 카드 (stock-card warning, data-section="sell-now"):

**없으면 이 섹션 통째로 생략 (h2 자체 출력 X)** — 빈 섹션 만들지 마라.

각 카드 형식:
- 헤더: "🚨 즉시 매도 검토 — 종목명 (티커)"
- live-price-row + 추천가 → 현재가 → 변동률 (당연히 -)
- 권고 1줄: "**손절가 도달 — 즉시 매도하고 다른 기회로 자금 이동**"
- 이유 1줄: "왜 손절해야 하나" — 노조 파업 격화, 손절선 깨짐, 어닝 미스, 주요 악재 등

<h2>🚫 오늘 절대 사지 말 것 (DO NOT BUY)</h2>

**대중이 따라 사기 쉽지만 지금은 위험한 종목 2~3개 카드** (stock-card warning, data-section="avoid"):

선정 기준:
- RSI 80 이상 과열 종목
- 52주 고점 근처 + 최근 +20% 이상 급등
- 단기과열 / 투자경고 종목
- 공매도 비율 급증 종목
- 주가 +5% 이상 갭상승하며 거래량 폭증 (FOMO 함정 가능성)

각 카드 형식:
- 헤더: "🚫 사면 위험 — 종목명 (티커)"
- 현재가 + 최근 1주 +N% 같은 컨텍스트
- 이유 2~3문장: 왜 위험한지 (RSI 87, 거품 신호, 추격매수 함정 등)
- "이 종목 사고 싶으면 RSI 50 이하 또는 -10% 이상 조정 후 검토"

<h2>🏆 오늘 살 거라면 — TOP 3 우선순위</h2>

**가장 중요한 섹션. 사용자가 페이지 맨 위만 보고 결정 가능하도록.**

전체 데이터(매수 검토 후보 + 발굴 후보 + 외국인 순매수 + 임원 매수 + 단타 후보)에서
신호 강도·리스크·기회 종합해서 **딱 3개**만 골라서 순위 매김.

각 순위마다 stock-card priority-N (data-section="top-priority", priority=1/2/3):

🥇 **1순위 — 종목명 (티커)** — 가장 강력한 매수 후보
- 신호 강도 (왜 1순위인지) — "임원 5명 동시 매수 + 외국인 N억 순매수 + 펀더멘털 PER 12 + 기술적 RSI 55 적정 + 노조 이슈 해결"
- 매수 가격대: ___원
- 1차 목표: ___원 (+8~10%)
- 2차 목표: ___원 (+18~20%)
- 손절: ___원 (-7~10%)
- 추천 비중: ___원 (전체의 N%)
- 위험: 무엇이 무너지면 이 시나리오 깨지는지
- 시간 horizon: 단타(1~3일) / 단기(1~2주) / 중기(1~3개월)
...
규칙:
- 1순위·2순위는 신호 강도 차이 명확하게 비교 ("1순위가 임원 매수 강도 5배 더 강함" 등)
- 3개 중 **1개는 반드시 발굴 후보(소형주)**에서. 대형주 3개로 채우지 마라.
- 순위 결정 근거를 명확하게: 왜 1순위가 1순위고 2순위가 2순위인가
- "추가 조사 필요" 라벨 종목은 3순위로 — 위험 명시 + 진입 전 DART/SEC 직접 확인 권고

<h2>📌 아침 추천 포지션 — 지금 상태</h2>

evaluated_positions 데이터 활용. 각 포지션마다 디테일 카드 (data-section="position-update"):
- 헤더: "📌 보유" / "✅ 1차 익절 도달" / "🚨 손절 도달" / "⚠️ 손절 임박" / "📉 단기 하락"
- live-price-row (추천가/현재가/변동률)
- stock-prices 영역 (매수가/익절1/익절2/손절가) — 본인이 매도 시점 비교용
- 권고: 명확하게
  - 손절 도달 → "**즉시 매도 검토 — 손해 더 커지기 전에**" (warning 카드 + 빨간 경고)
  - 손절 임박 → "**손절가 근접 — 매도 준비. -X% 더 빠지면 즉시 매도**"
  - 1차 익절 도달 → "**일부(50%) 매도 검토. 나머지 50%는 2차 목표까지 보유**"
  - 2차 익절 도달 → "**대부분(80%) 매도 검토. 잔여는 추가 상승 노림수**"
  - 보유 → "**보유 유지. 다음 갱신까지 관찰**"
- stock-reason 2~3문장: 어떤 흐름이라 이 권고인지 설명 (뉴스, 시장 분위기, 기술적 등)

evaluated_positions 비어있으면 "오늘 아침 풀 분석 데이터 없음 — 새 추천부터 시작" 한 줄로 끝.
